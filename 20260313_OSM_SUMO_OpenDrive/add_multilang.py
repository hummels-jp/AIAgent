...
def process_file(filepath):
    ...
    for chinese in sorted_keys:
        # 使用正则表达式,避免替换HTML标签和代码块中的内容
        # 只替换不在HTML标签和代码块中的文本
        pattern = r'(?<!&lt;[^&gt;]*)(?<![>"])\b' + re.escape(chinese) + r'\b(?![^<]*&gt;)(?!["\'])'

        def replacer(match):
            # 检查是否已经替换过
            if match.group(1) is not None and 'zh-lang' in match.group(0):
                return match.group(0)
            return create_multilang_span(chinese)
        ...
        if old_count > 0:
            ...
            replacements += old_count - content.count(new_str.replace(chinese, ''))
